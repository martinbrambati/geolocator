...
//	print_r($config);
	...
	echo "activationURL=".$config['activation_url'].$sep."hashID=".$id.$sep."maxCoordsPerPost=".$config['max_coords_per_post'].$sep."maxCoordsReadFromGps=".$config['max_coords_from_gps'].$sep."postURL=".$config['post_url'].$sep."returnHeight=".$config['return_height'].$sep."updatePositionTime=".$config['update_position_time'].$sep."updateConfigTime=".$config['update_config_time'].$sep."enableLogging=".$config['enable_logging'].$sep."updatePostTime=".$config['update_post_time'];
?>
